...
def loadTargetUserIDs(filename):
	linecount = 0
	target_users_id = []
	with open(DATAPATH+'/target/'+filename,'rb') as f:
		reader = csv.reader(f)
		for row in reader:
			if linecount > 0:
...
with open(DATAPATH+'/solution/solution_svm.csv', 'w') as f:
...
for user in target_user_ids:
	...
	# writing process
	interactions = {}
	interactions_tmp = getInteractionsByUserAndWeek(user,40)
	...
	impressions =  loadImpressions(user,40)
...
# 	itemset = arr_json['items']
	...
	if Y is None or len(set(Y)) == 0:
		...
		maincounter += 1
		if maincounter % 10 == 0:
			with open(DATAPATH+'/solution/solution_svm.csv', 'a') as f:
				for line in output:
					f.write(str(line)+"\n")
